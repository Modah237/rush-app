...
export default function CourierPage() {
  ...
  return (
    <div className="page-enter flex flex-col gap-5 px-4 md:px-0 mt-4 pb-12">
      {/* Header Livreur */}
      <div className="bg-gradient-to-br from-brand to-brand-active text-white p-5 rounded-lg flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-warm-2">
        <div>
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className={`w-12 h-6.5 rounded-full p-1 transition-colors cursor-pointer flex items-center ${
                isAvailable ? 'bg-success justify-end' : 'bg-white/20 justify-start'
              }`}
            >
      ...
      {verificationStatus === 'unsubmitted' && (
        <form onSubmit={handleSubmitIdentity} className="card bg-surface p-5 rounded-lg border border-border-warm shadow-warm-1 flex flex-col gap-4">
          ...
          <p className="text-[13.5px] text-ink-muted leading-relaxed">
            Pour devenir livreur officiel sur la plateforme RUSH et commencer à livrer, veuillez renseigner vos informations de véhicule et fournir vos pièces justificatives obligatoires de sécurité.
          </p>
      ...
      {verificationStatus === 'pending' && (
        <div className="card bg-surface p-6 rounded-lg border border-border-warm shadow-warm-1 text-center flex flex-col items-center max-w-lg mx-auto py-12">
          <span className="w-16 h-16 rounded-full bg-amber-tint text-warning flex items-center justify-center">
          ...
          {activeOrders.map((o) => (
            <div key={o.id} className="card bg-surface p-4.5 rounded-lg shadow-warm-1 flex flex-col gap-3.5">
              ...
              <div className="text-[13px] text-ink-light leading-relaxed">
                ...
                {o.note && <div className="mt-1.5 text-warning font-bold bg-amber-tint/40 p-2 rounded-md border border-warning/10">Note : "{o.note}"</div>}
              </div>
